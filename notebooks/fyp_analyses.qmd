...
ci_fyp <- pp_fyp_tidy |> 
  ...
  mutate(model = factor(model, 
                        ...
                        labels = c("Cellular Communications (Metadata)", "Null")),
         y = 700) |> 
  filter(model == "Cellular Communications (Metadata)")
```
...
pp_fyp_tidy |> 
  ...
  filter(model == "Cellular Communications (Metadata)") |> 
  ggplot() + 
  geom_histogram(aes(x = posterior), color = "#0B5345", 
                 fill = "#A2D9CE",
                 bins = 60) +
  geom_segment(mapping = aes(y = y+100, yend = y-100, x = pp_median, xend = pp_median,
                           ), color = "black",
               data = ci_fyp) +
  geom_segment(mapping = aes(y = y, yend = y, x = pp_lower, xend = pp_upper), color = "black",
                data = ci_fyp) +
