...
![](https://github.com/KendraPaquette/presentations/blob/main/figs/mendota.jpg?raw=true) 
:::
...
![](https://github.com/KendraPaquette/presentations/blob/main/figs/loading.jpg?raw=true) 
:::
:::
...
![](https://github.com/KendraPaquette/presentations/blob/main/figs/cpa.png?raw=true) 
:::

:::{.column width = "50%"}
:::{.fragment}
![](https://github.com/KendraPaquette/presentations/blob/main/figs/protocol.png?raw=true) 
:::
:::
...
![](https://github.com/KendraPaquette/presentations/blob/main/figs/calendar.png?raw=true){.absolute height="80%"}  
:::
...
:::{.notes}
I originally proposed a 6 chapter, 4 study dissertation. I am going to give you an update on my progress with study 3, but as a heads up at the end I am going to propose revising the scope to be a more traditional 5 chapter dissertation.
:::
...
3. <span style="color: #c5050c;">**Predict and characterize *immediate* (next-day) opioid lapse risk from geolocation and daily surveys using machine learning methods.**</span>
...
## Study 3: Design
- Provided continuous geolocation data
- Contextualized frequently visited locations
...
:::{.notes}
Provided continuous geolocation data
Each month we asked participants self-report questions about their frequently visited locations. 
:::
...
-   Missing data: mean=.02, range = 0-.11


:::{.notes}
The proportion of missing values across features was low
:::
...
-   Total of 88,607 day-level labels
...
Forty percent of participants reported an opioid lapse while on study. This resulted in 1.81% of the labels positive for lapse 
:::
...
Fit: elastic net, XGBoost (reporting on XGBoost), Plan to also look at: random forest, a single-layer neural network, and a support vector machine
...
:::{.notes}
Feature importance values provide insight into the features that have the most influence on the model’s predictions. For every prediction, we can extract feature importance values providing actionable insight into intervenable targets for lapse risk (i.e., for a specific individal at a specific moment). We calculated raw Shapley values for each observation in the held-out test sets.

Here I am showing you Shapley values aggregated across all participants and all observations to provide a relative rank ordering of the most important features.

However, it can be skewed toward features that exhibit infrequent but very large Shapley values, potentially overstating the importance of features that are strongly associated with the outcome, but only come up in a small subset of observations. This is most notably with lapse. Lapses were very infrequent but when they did occur they increased lapse probability tremendously.
:::
...
The second way we calculated feature importance was by calculating the proportion of study days that each feature emerged as the most important feature contributing to lapse. This approach summarizes how frequently a feature category is influential across observations (i.e., considering both magnitude and prevalence). 

I am really excited about understanding the features contributing to lapse and how features can be mapped on to recovery recommendations. 
- Also looking at individual shapley values to understand the direction of their relationship to risk
- I am interested in exploring interactions between features.
:::
...
:::{.notes}
to pursue exploring these interests more deeply and due to applying to internship this year instead of next year....

Still plan to carry out study 4 but would like to do it on a different timeline from the rest of my dissertation.
:::
