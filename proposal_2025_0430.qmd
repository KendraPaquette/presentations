...
:::{.notes}
In study 3 I am proposing to use similar methods from the previous studies to predict immediate lapses back to opioid use. 
...
The first aim is to evaluate the performance of a machine learning model that predicts opioid lapse risk from geolocation and daily surveys.

The 24-hour prediction windows roll day-by-day starting at 6 am in the participant’s own time zone starting on day 7. 

We will fit xgboost model configurations and use grouped nested k-fold cross-validation for model selection and evaluation.
...
:::{.notes}
Our second aim is to Assess model fairness in performance across important subgroups.
...
We will conduct Bayesian group comparisons to assess the likelihood that each model performs differently by group.
:::
...
I will use shapley values as I did in the previous studies to understand how important features are on average and individually to the model's predictions. I expect we will see new important features emerge from the expansion of our EMA items and the addition of geolocation data. 
:::
...
:::{.notes}
So up until this point I have only been focussed on using traditional machine learning algorithms for predicting lapses. While there are a lot of benefits for prediction with these algorithms, a notable limitation when it comes to predicting infrequent outcomes like lapses, is that the models are not built using an individuals own data. 
...
We will use grouped k-fold cross validation to generate group-level Bayesian priors from participants in the held-in data. These priors are more important for early labels where less data is available for a specific individual. 

We will Report the Median posterior probability for auROC and Bayesian CI for our state-space model and conduct a bayesian model comparison where we compare the performance of the state-space models to our traditional machine learning model in study 3. 
:::
...
:::{.notes}
A benefit of time series models, like state-space models, is that they could potentially improve the efficiency and performance of lagged prediction models. A single model can be used to predict a lapse in the next day or at any point in the future.
...
:::{.notes}
We could imagine that personalized models could be another solution for mitigating fairness issues previously seen, as they weight the individuals data more heavily than group estimates. Therefore we will also explicitly Assess model fairness in model performance for immediate lapses across the same subgroups from study 3. 
:::
...
:::{.notes}
5 selected other: Multiracial, Native American, Arab, Jewish, God
:::


## Race/Ethnicity {.scrollable}

10 participants selected 2 or more races
```{r}
#| echo: false
...
- Always using EMA from day before for lapse predictions
